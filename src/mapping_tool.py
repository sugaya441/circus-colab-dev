...
class MappingTool(tk.Frame):
    ...
    def save_mapping_to_file(self):
        ...
        # data_saver.py 用に「管理番号の文字列」→「管理番号」列を補完
        if '管理番号の文字列' in new_df.columns:
            new_df['管理番号'] = new_df['管理番号の文字列']

        if '企業名' not in new_df.columns or '管理番号の文字列' not in new_df.columns:
            print("[保存エラー] '企業名'または'管理番号の文字列'列が存在しません")
            return

        if not existing_df.empty:
            target_keys = set(zip(new_df['企業名'], new_df['管理番号の文字列']))
            keep_df = existing_df[~existing_df.apply(lambda row: (row['企業名'], row['管理番号の文字列']) in target_keys, axis=1)]
